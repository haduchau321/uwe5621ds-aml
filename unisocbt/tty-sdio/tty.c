...
unsigned long log_level = LOG_LEVEL_WARNING ;

static ssize_t bt_loglevel_show(struct device* dev,
    struct device_attribute* attr,
    char* buf)
{
	unsigned long ret;
	...
	return sprintf(buf, "%ld", ret);
}

static ssize_t bt_loglevel_store(struct device* dev,
    struct device_attribute* attr,
     const char* buf, size_t count)
{
	unsigned long loglevel;
	int ret;
	ret = kstrtoul(buf,10,&loglevel);
	BT_WARNING("%s--%d\n",__func__,ret);
...
static DEVICE_ATTR(bt_loglevel, 00660, bt_loglevel_show, bt_loglevel_store);
