...
void class_dis(void);
unsigned char resume_rxmsg[7] = { 0x04, 0x05, 0x04,0x00, 0x10, 0x00, 0x13 }; //
...
unsigned char class_resume_rxmsg[7]={0x04,0x05,0x04,0x00,0x00,0x00,0x13}; //classic  disconnect_event
...
extern unsigned char red_bt_classic_flag;
...
static int bt_tx_powerchange(int channel, int is_resume)
{
    ...
    if (strcmp(WOBLE_TYPE, "disable")) {
		...
		if ( red_bt_classic_flag == BT_CLASSIC_CONNECTED)
                {
                    resume_transfer_upper(class_resume_rxmsg, 7);
                }
                pr_err("resume_null2...\n"); //
#endif
