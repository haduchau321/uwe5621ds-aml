...
extern unsigned char class_resume_rxmsg[7]; //classic  disconnect_event
extern int i;
int num = 9;
unsigned char red_bt_flag=0x11; //区分蓝牙，红外待机标志；0x11:红外 0x22: bt
unsigned char red_bt_classic_flag = BT_CLASSIC_DISCONNECTED;
unsigned char BT_Conn_Flag = 0;
// extern int event_nu;
    ...
    int (*upper_cb)(const unsigned char* buf, int count))
{
    ...
    do {
		...
		case BT_HCI_EVT_DIS_COMPLETE:   //BT_HCI_EVT_DIS_COMPLETE
                // res = *(rxmsg+6);
                if ((dis_flag == 0x64) && (*(rxmsg + 4) == resume_rxmsg[4]) && (*(rxmsg + 5) == resume_rxmsg[5])) {
                    ...
                    upper_cb(rxmsg, pack_length);
                } else if( (*(rxmsg + 4) == class_resume_rxmsg[4]) && (*(rxmsg + 5) == class_resume_rxmsg[5]) ) {
                    red_bt_classic_flag = BT_CLASSIC_DISCONNECTED;
                    upper_cb(rxmsg, pack_length);
                }
                else   //其他设备断
                {
		...
		case BT_HCI_EVT_VENDOR_SPECIFIC:
                if ((BT_HCI_CONNECT_REQ_ERROR == rxmsg[EVT_VENDOR_CODE_LSB])
		    || (BT_HCI_INQUIRY_ERROR == rxmsg[EVT_VENDOR_CODE_LSB])) {
                    rxmsg_shadow[PACKET_TYPE] = HCI_EVT;
                    rxmsg_shadow[EVT_HEADER_EVENT] = BT_HCI_EVT_HARDWARE_ERROR;
                    rxmsg_shadow[EVT_HEADER_SIZE] = 0x01; //size
                    rxmsg_shadow[3] = 0x00; //parameter
                    pack_length = rxmsg_shadow[EVT_HEADER_SIZE];
                    pack_length += 3;

                    /* Send upper Hardware Error event */
                    upper_cb(rxmsg_shadow, pack_length);
                    pr_err("Send upper Hardware Error event. \n");
                }
                else {
                    pack_length = rxmsg[EVT_HEADER_SIZE];
                    pack_length += 3;

                    /* Send upper Vendor Specific event */
                    upper_cb(rxmsg, pack_length);
                    pr_err("Send upper Vendor Specific event. \n");
                }
                break;

		case BT_HCI_EVT_LE_META:   //BT_HCI_EVT_LE_META
                test_buf = rxmsg + 3;
                ...
                if (Connection_Complete == sub_code || LE_Enhanced_Connection_Complete == sub_code) {
                    ...
                    if ( hci_status == HCI_SUCCESS )   //status
                     {
		...
		case BT_HCI_EVT_CLASSIC_CONNECT_COMPLETE:   //BT_HCI_EVT_CLASSIC_CONNECT_COMPLETE
                	hci_len = *(rxmsg + 2) ;  // 11
			hci_status = *(rxmsg + 3) ; //00:success; 01:faile
			if ( (hci_len == CLASSIC_CONNECT_COMPLETE_LEN) && (hci_status == HCI_SUCCESS))
			{
				class_resume_rxmsg[4] = *(rxmsg + 4);
                		class_resume_rxmsg[5] = *(rxmsg + 5);
				red_bt_classic_flag = BT_CLASSIC_CONNECTED;
				pr_err("%x----%x\n",class_resume_rxmsg[4],class_resume_rxmsg[5]);
				pr_err("classic connect success!\n");  //bluetooth
			}
			else
			{
				red_bt_classic_flag = BT_CLASSIC_DISCONNECTED;
				pr_err("classic connect is not success!\n");
			}
...
void hci_disconnect(void)
{
    ...
    UINT8_TO_STREAM(p, HCI_ERR_CONN_CAUSE_LOCAL_HOST); //0x16
...
//send classic device disconnect
void class_dis(void)
{
    ...
    payload->len = 3;
    UINT8_TO_STREAM(p,  class_resume_rxmsg[4]);     //
    UINT8_TO_STREAM(p,  class_resume_rxmsg[5]);  //
    ...
    hci_cmd_send_sync(BT_HCI_OP_DISCONNECT, payload, NULL);
    vfree(payload);
    pr_err("class_dis mmsg[4]=%d\n", class_resume_rxmsg[4]);
    pr_err("class_dis mmsg[5]=%d\n", class_resume_rxmsg[5]);
    pr_err("%s end\n", __func__);
    return;
}
