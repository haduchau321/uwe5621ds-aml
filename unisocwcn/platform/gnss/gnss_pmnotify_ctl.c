...
/* Replace PM notifier with suspend/resume callbacks */
static int gnss_suspend(struct device *dev)
{
    int i = 10;
    ...
    gnss_delay_cancel = 0;
    while (gnss_delay_ctl() && (i--) && (gnss_delay_cancel != 1)) {
        mdelay(2);
    }

    pr_info("%s: GNSS suspend\n", __func__);
    return 0;
}

static int gnss_resume(struct device *dev)
{
    gnss_dev.gnss_flag_sleep = false;
    pr_info("%s: GNSS resume\n", __func__);
    return 0;
}
